...
Such images will easily allow to generate foreground mask from non-transparent pixels. If not, tools like GIMP~\cite{howat2014greenland}, 
combined with deep learning forground extractors (Authors used remove.bg~\cite{website reference} that uses a combination of Image based techniques 
and DNN to separate foreground from background) can help generate the required PNG foreground images.

% ADD FIGURES for cattle as an example to show sample foreground and background images
...
For $k=20$ and $n=m=100$ this becomes $400,000$ fg-bg images. Algorithm \ref{algo:datagen} descrbes the data generation process
...
\begin{algorithm*}
  ...
  \Input{$m$ Background Image paths, $n$ Foreground Image paths, multiplexing factor $k$, batch size $b$ \emph{must be multiple of $k$}}
...
A maximal random crop of $448 X 448$ without affecting the image aspect is done on 
